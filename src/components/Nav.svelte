...
<header>
  ...
  <div class="container top">
    <a href="/" class="brand">
      ...
      <svg viewBox="0 0 512 128" fill="none" xmlns="http://www.w3.org/2000/svg">
        <title>Navillus</title>
        <circle cx="64" cy="64" r="64" fill="var(--chisel-neutral-900)" />
        <rect
          x="44.1567"
          y="38.5"
          width="64"
  ...
  {#if !mounted || menuOpen}
    ...
    >
      <ul>
        {#each routes as { title, href } (title)}
          <li>
            <a {href} class="h5" aria-current={pathname === "/"}>{title}</a>
          </li>
        {/each}
        <li>
          <a
